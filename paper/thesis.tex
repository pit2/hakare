...
\begin{document}
    ...
    Handwritten optical character recognition (HOCR) is a task commonly solved with deep learning methods. One of the most classic examples in machine learning is the MNIST dataset that contains images of handwritten characters from the set $\{0, 1, \dots, 9\}$ \todo{ref}. Although this is a tame set in the sense that the images have already been pre-processed \todo{ref book}, in general, HOCR is a difficult problem:
    \begin{itemize}
    ...
    We optimised above set of hyperparameters using an elitist genetic algorithm \todo{reference} with population size 20, crossover probability 0.9, swapping probability 0.5 and mutation probability. The objective function to be maximised is the accuracy on the test set. 
